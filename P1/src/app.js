...
app.get('/OAuth', function(req, res){
    ...
    var serviceUrl = services_vcap.OauthServiceBroker[0].credentials.serviceUrl+"/";

    var provider = req.query.provider;

    //Calling /facebook|/twitter|/google|/linkedin service of OauthService
...
app.get('/callback', function(req, res){
    ...
    if(req.query.code){
        ...
        var services_vcap = JSON.parse(process.env.VCAP_SERVICES || "{}");
        var serviceUrl = services_vcap.OauthServiceBroker[0].credentials.serviceUrl+"/account";
        var apiKey = services_vcap.OauthServiceBroker[0].credentials.apiKey;
        var body = {
            code : req.query.code,
            apiKey : apiKey
        }
...
app.get('/logout', function(req, res){
    ...
    //Calling /logout service of OauthService
...
app.post("/generateOTPWithTwilio", function(req, res){
    var services_vcap = JSON.parse(process.env.VCAP_SERVICES || "{}");
    var serviceUrl = services_vcap.OauthServiceBroker[0].credentials.serviceUrl+"/generate";
    request({
        url: serviceUrl, //URL to hit
        method: 'POST',
        //Lets post the following key/values as form
...
app.post("/validateOTP", function(req, res){
    var services_vcap = JSON.parse(process.env.VCAP_SERVICES || "{}");
    var serviceUrl = services_vcap.OauthServiceBroker[0].credentials.serviceUrl+"/validate";
    request({
        url: serviceUrl, //URL to hit
        method: 'POST',
        //Lets post the following key/values as form
